...
begin 
	# Add labels/ svgs
	ax_2x2_12 = Axis(fig_2x2[2,2:3], width=Relative(1), height=Relative(0.25), halign=0.0, valign=0.,limits=((0,1),(-0.5,2.5)))
	...
	for (i, ax) in enumerate(all_ax)
		...
		scatter!(ax,1,2.2,marker=get_unfold_svg(all_svg[tmp_mark]),markersize=size_marker)

		tmp_mark = all_marks[i][2]
		scatter!(ax,1,2.0,marker=get_unfold_svg(all_svg[tmp_mark]),markersize=size_marker)

		tmp_mark = all_marks[i][3]
		scatter!(ax,4,2.2,marker=get_unfold_svg(all_svg[tmp_mark]),markersize=size_marker)
	end
...
manifest_format = "2.0"
project_hash = "d52495cffe5366c862c144aef7b4fcb250fea494"
