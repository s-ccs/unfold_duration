...
# ╔═╡ dc5213e8-bde5-11ee-2053-a9888eda731e
begin
	using Unfold,DSP,UnfoldSim
	using CairoMakie
...
# ╔═╡ d3862bcf-7f9d-45f9-b70f-28d61b860d18
begin
	# include SVGs
	using Rsvg,Cairo
using PlutoLinks
...
begin
	 svg_modality_eeg = "https://cloud.wirdreibei.de/s/C86ccGj7a37ebDQ/download/modality_eeg.svg" |> download|> x->read(x,String) |> SVGDocument#Rsvg.handle_new_from_file
	svg_modality_bold = "https://cloud.wirdreibei.de/s/FaqEb6yrqk2dCoD/download/modality_bold.svg" |> download|> x->read(x,String) |> SVGDocument# Rsvg.handle_new_from_file
	svg_modality_pupil = "https://cloud.wirdreibei.de/s/N4SYzXC5x9SA4tJ/download/modality_pupil.svg" |> download|> x->read(x,String) |> SVGDocument# Rsvg.handle_new_from_file
	
	    scatter(0.2,0,marker=svg_modality_eeg,markersize=100)
	scatter!(0.3,0,marker=svg_modality_bold,markersize=100)
	scatter!(0.4,0,marker=svg_modality_pupil,markersize=100)
	current_figure()
end
...
# ╔═╡ 070d16c6-ad93-435e-82dc-db89e5669e29
	...
	function p3_scale(l)
		p3_end = hanning(Int(round(l)))
		return p3_end[end÷2:end]
...
begin
	...
	function plot_eeg(ax,ax2)
	srate = 1000
y = zeros(Int(1.2*srate) )
p1 = p100(sfreq=srate)*1.8
n1 = n170(sfreq=srate) *1
p3 = hanning(Int(.4*srate))
p3_stop = length(p3)÷2
p3[p3_stop:end] .= 0
...
y[1:length(n1)] .+= n1
	p3_start_n1 = 0.05
y[length(n1)-Int(p3_start_n1*srate):length(n1)+length(p3)-1-Int(p3_start_n1*srate)] .+= p3
p3dur = [.50,.80,1.10,1.40]*srate
...
# ╔═╡ 8f38da8a-f144-4681-b644-687d424f3ef4
md"""
## fmri / scaled hanning
"""

# ╔═╡ 22536488-9689-44f1-baa3-157365c516b7


# ╔═╡ dc3416d7-02e6-40bc-a5c6-5eb58f5c125a
function fmri_resp(eventonset;TR = 1.,delayResp = 6,delayUnder=16,p3=1,p4=1,p5=16,p6=0,p7=32)
bold = hrfbasis(TR;parameters=[delayResp,delayUnder,p3,p4,p5,p6,p7]) # using default SPM parameters
bold_kernel = e->Unfold.kernel(bold,e)
	resp = bold_kernel(eventonset)[:,1]
	resp = resp ./ maximum(resp)
return resp
end

# ╔═╡ 01afdc58-3345-4038-9ee1-f6e87e56568d
begin
	function plot_bold(ax1,ax2)
	y = zeros(100) # signal length = 100
	sig_durations = [5,20,2,10]
	sig_starts = [10,30,60,75]
	y[vcat([start:start+dur for (start,dur) in zip(sig_starts,sig_durations)]...)] .=1 
	
	
	y_conv = conv(y,fmri_resp(0,TR=0.5)) # convolve!

	function single_resp(duration;offset=0)
		y = zeros(100)
...
# ╔═╡ 143b4029-1d77-4a7a-9b04-60272cf06dab
md"""
## pupil / hanning
"""

# ╔═╡ 0a870a1d-abec-4b5e-98fb-23df6a00d5b0
begin
	...
	xlims!(ax_cont,[0,2.5])
	ax_cont.xlabel = "time [s]"
	ax_cont.ylabel = "pupil [a.u.]"
	end
...
begin
#f = Figure(;size=(10,200))
	f = Figure(resolution=(800,400))
	
	icon_list = [svg_modality_pupil,svg_modality_bold,svg_modality_eeg]
	for (mod,icon,labelstring) = zip(1:3,icon_list,["A)", "B)", "C)"])
		ax_icon = f[mod,1] = Axis(f,limits = ((-2,4),(-0.5,1.3)))
		hidedecorations!(ax_icon)
		hidespines!(ax_icon)
		ax_resp = f[mod,2] = Axis(f)
		hidedecorations!(ax_resp)

		ax_deco = f[mod,3:4] = Axis(f,yaxisposition=:right)

				ax_deco.yticklabelpad = 0
			#ax_deco.yticklabelalign=(:right,:center)
			ax_deco.xticklabelpad = -7
		#hidedecorations!(ax_deco,ticks=false)
		#hidespines!(ax_deco,:l)
		ax_deco.rightspinevisible = true
		ax_deco.xgridvisible = false
		ax_deco.ygridvisible = false
		#ax_deco.xlabelpadding = -23
		linkyaxes!(ax_resp,ax_deco)
		if mod == 3
			plt_f = plot_eeg
			ax_deco.ylabel="ERP [µV]"
			ax_deco.yticks = [0,2]

			ax_deco.xticks = ([0,1],["0s","1s"])
			
		elseif mod ==2
			plt_f = plot_bold
			ax_deco.ylabel = "BOLD [a.u.]"
			ax_deco.yticks = [0,10]
		#ax_deco.yticklab
			ax_deco.xticks = ([0,50,100],["0s","50s","100s"])
		elseif mod ==1
			plt_f = plot_pupil
			ax_deco.ylabel = "Pupil [a.u.]"
			ax_deco.yticks = [0,-1]
			ax_deco.xticks = ([0, 1 ,2],["0s","1s","2s"])
		end
		
		plt_f(ax_resp,ax_deco)
		#if mod !=3
			ax_deco.xlabel = ""
		#end
...
manifest_format = "2.0"
project_hash = "7b7700275ea0de99ed3746b262837019a86bdebd"
...
[[deps.MakieTeX]]
deps = ["Base64", "Cairo", "Colors", "DocStringExtensions", "Ghostscript_jll", "Glib_jll", "LaTeXStrings", "Makie", "Poppler_jll", "Rsvg", "Typstry", "tectonic_jll"]
git-tree-sha1 = "e7f4f5d4f70519eb5eccb685eb46cda84ac93bc4"
uuid = "6d554a22-29e7-47bd-aee5-0c5f06619414"
version = "0.4.3"
weakdeps = ["CairoMakie"]

    [deps.MakieTeX.extensions]
...
git-tree-sha1 = "bfafe9d050ac0a96b720b9cd078a9d753c761b94"
