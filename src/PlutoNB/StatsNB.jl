...
begin
	...
	noOVplt1 = data(ovDF) * visual(Violin) * mapping(:shape, :MSE_nodc, color=:formula, dodge=:formula)
	#plt = data(filtDF) * visual(BoxPlot, show_notch=true) * mapping(:shape, :normMSE, color=:formula, dodge=:formula, layout=:overlap)
...
# ╔═╡ 1558783c-aa5a-49ba-a946-fb34ca4cdc3d
	tmpDF = DataFrame(
						layout = [repeat(["normMSE"], size(ovDF, 1)); repeat(["normMSE_nodc"], size(ovDF, 1))],
						MSE = [ovDF.normMSE; ovDF.normMSE_nodc],
						shape = [ovDF.shape; ovDF.shape],
						formula = [ovDF.formula; ovDF.formula]
					)
...
# ╔═╡ fa3944da-43be-4eca-a183-35cdfb5a2101
filtDF

# ╔═╡ 356dd163-fbad-42fa-b0f1-e94e656cbefe
#save("normMSE_violin.eps", fig)

# ╔═╡ e4cc7911-92e8-4894-a83b-f9d8fc428b53
begin
	filt2DF = df[(df.noise .== "noise-1.00") .& 
				(df.overlap .== "overlap-1") .& 
				(df.durEffect .== "durEffect-1") .& 
				(df.formula .!= "theoretical") .& 
				(df.formula .!= "y~1"), :]
	
	xy = data(filt2DF) * mapping(:shape, :normMSE, color=:formula, row=:formula, col=:overlapdist)
