...
# ╔═╡ 0b18916e-0614-11ee-2ef3-03dafe051b73
begin
	using CSV
	using Printf
...
# ╔═╡ a267e327-b636-43b3-bef2-9328772afe1f
begin
	eventsAll = DataFrame()
	for sub in 1:40
		@show sub
		tmp_events = []
		try
	    	tmp_events = CSV.read(@sprintf("/store/projects/unfold_duration/local/P3_clean/csv/%i_P3_shifted_ds_reref_ucbip_hpfilt_ica_weighted_clean.set.csv",sub),DataFrame, delim=",");
		catch
			continue
		end
		tmp_events.subject .= sub
		append!(eventsAll, tmp_events)
	
	end
end

# ╔═╡ 04bac9fa-71f4-4805-890c-0837745cf9de
eventsAll

# ╔═╡ 2ed8a5fc-58d4-4216-8dbd-9f62e70d0b55
function RT_DF(events_df)

	rt_df = DataFrame()
	for r = 1:nrow(events_df)

		if events_df[r,:].trialtype == "target"
			continue
		end

		rt = events_df[r,:].rt
		if (events_df[r,:].trialtype == "distractor" && r == 1)
			nback = "none"
		elseif  r > 1 && events_df[r-1,:].trialtype == "target"
			nback = "1back"
		elseif  r > 2 && events_df[r-2,:].trialtype == "target"
			nback = "2back"
		elseif  r > 3 && events_df[r-3,:].trialtype == "target"
			nback = "3back"
		else
			nback = "none"
		end
		tmp_df = DataFrame(nback = nback, rt = rt)
		append!(rt_df, tmp_df)
	end
	return rt_df
end

# ╔═╡ 6f12dc26-e28d-4927-b12a-f16bcdf85959
begin
	rt_dfAll = DataFrame()
	for r = unique(eventsAll.subject)
		sub_events = @subset(eventsAll, :subject .== r, :eventtype .== "stimulus")
		rt_df = RT_DF(sub_events)
		rt_df.subject .= r
		append!(rt_dfAll, rt_df)
	end
end

# ╔═╡ c4e63fef-ff0a-407c-84cf-22c64ceb2b48
begin
	#f = Figure()
	data(rt_dfAll) * mapping(:rt, color=:nback) * AlgebraOfGraphics.density() |> x -> draw(x)
	#legend!()
	
end

# ╔═╡ 13d84537-1bd7-498e-b900-68db1c73f193
let
	#f = Figure()
	data(rt_dfAll) * mapping(:rt, color=:nback) * AlgebraOfGraphics.density() |> x -> draw(x)
	#legend!()
...
# ╔═╡ c0005490-571b-42ae-9886-3b5c85346c1b
let
	f = Figure()
	for (i,s) in enumerate(7:10)
	df = @subset(rt_dfAll, :subject .== s)
	data(df) * mapping(:rt, color=:nback) * AlgebraOfGraphics.density() |> x -> draw!(f[i,1],x)
		xlims!(current_axis(), [50 1000])
	#legend!()
	end
	f
end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "7e073f7c2ad7580174d64fda932ebba20b7588df"
