...
# ╔═╡ 29abc041-9130-4387-9384-88aa03fcab90
begin
	using CSV
	using DataFrames
...
# ╔═╡ baae29b2-8b59-49c6-802e-8faec5aa7b2a
md"""
# Can modelling duration effects replace overlap correction?
"""

# ╔═╡ 61862a2e-abfa-11ef-2d13-37fb7fe7a37f
begin
	# Load data (including base line correction)
	folder = "20240618_sim_realNoise_HanningShapes_filtered_blockdesign"

	fpath = joinpath("/store/projects/unfold_duration/local/simulationResults/", folder, "simulationResults_" * folder * "_MSE_bsl.csv")
	
	df = CSV.read(fpath, DataFrame)
end

# ╔═╡ 8e373417-6cc9-4b31-9810-e2978ed60073
begin
	# Extract data for the case in which there is overlap but no duration effect
	# simulated
	data_sub = @rsubset(df,
			:shape == "scaledHanning",
			:noise =="noise-1.00",
	...
	sorting_dict = Dict("y~1" => 1,"y~1+dur" => 2, "y~1+cat(durbin)" => 3, "y~1+spl(dur,5)" => 4, "y~1+spl(dur,10)" => 5)
	int_vector = [sorting_dict[f] for f in data_sub.formula]
end;

# ╔═╡ 035e444b-c903-43b1-b36d-cf700ba96d73
save_figure = false

# ╔═╡ 35ed200c-de74-438f-835a-76ecd7d4144b
md"""
## Without baseline correction
"""

# ╔═╡ f2d0cce5-5bae-4842-a10c-4a5df1b575e7
let
	f = Figure()
	ax = Axis(
		f[1,1],
		xticks = (collect(values(sorting_dict)), collect(keys(sorting_dict))),
		title = "Overlap simulated + No duration effect",
		ylabel = "Model performance [MSE]")
	boxplot!(ax, int_vector, data_sub.MSE, label = "Overlap correction")
	boxplot!(ax, int_vector, data_sub.MSE_nodc, label = "No overlap correction")
	axislegend(position = :lt)

	if save_figure
		# Determine current date
		date = Dates.format(Dates.today(), "yyyymmdd")
		CairoMakie.save(date * "_DurationModelling_vs_OverlapCorrection_no_bsl.pdf", f)
	end
	
	current_figure()
end

# ╔═╡ 602273c8-d2a1-44f4-a06e-f5e853937ad1
md"""
## With baseline correction
"""

# ╔═╡ 0e300357-1c4c-422d-b827-38f53a35b624
let
	f = Figure()
	ax = Axis(
		f[1,1],
		xticks = (collect(values(sorting_dict)), collect(keys(sorting_dict))),
		title = "Overlap simulated + No duration effect (With baseline correction)",
		ylabel = "Model performance [MSE]")
	boxplot!(ax, int_vector, data_sub.MSE_bsl, label = "Overlap correction")
	boxplot!(ax, int_vector, data_sub.MSE_nodc_bsl, label = "No overlap correction")
	axislegend(position = :lt)

	if save_figure
		# Determine current date
		date = Dates.format(Dates.today(), "yyyymmdd")
		CairoMakie.save(date * "_DurationModelling_vs_OverlapCorrection_bsl.pdf", f)
	end
...
manifest_format = "2.0"
project_hash = "de2df5d74cee355b0f564e60011afb6d2884f710"
