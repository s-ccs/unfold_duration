...
begin
	...
	plt3 = data(noiseDF) * visual(BoxPlot) * mapping(:shape, :normMSE, color=:formula, dodge=:formula, col = :noise)
...
begin
fig_3 = Figure()
ax_31 = Axis(fig_3[1,1], title = "No overlap correction")
ax_32 = Axis(fig_3[2,1], title = "With overlap correction")
ax_33 = Axis(fig_3[:,2], title = "With OC and duration effect")
...
begin
	plt_shapes = data(@rsubset(df,
			...
			:formula != "theoretical",
			:formula !="y~1",
	)) * visual(BoxPlot) * mapping(:shape, :normMSE, color=:formula, dodge=:formula) + data((x=[0, 4], y=[1, 1])) * visual(Lines) * mapping(:x, :y) + data((x=[0, 4], y=[0, 0])) * visual(Lines) * mapping(:x, :y)
