...
# ╔═╡ 8862955f-e207-402b-9e84-ee6cdd1cd616
#html"""
#<style>
#  main {
#   max-width: 1500px;
...
begin
	...
	values = [1, 2, 3]
	labels = ["hanning", "pos Half", "scaled Hanning"]
	
	resolution = (1500, 600)
	fig = Figure(; resolution)
	ax = Axis(fig[1, 1], title = "With OC")
	
	#plt = data(filtDF) * visual(Violin) * mapping(:shape, :normMSE, color=:formula, dodge=:formula)
	fontsize_theme = Theme(fontsize = 24)
	set_theme!(fontsize_theme)
	plt = data(filtDF) * visual(BoxPlot) * mapping(:shape, :normMSE, color=:formula, dodge=:formula)

	draw!(ax, plt; axis = axis = (; xticks = (values, labels)))
	ylims!(-0.2, 2.3)
	hlines!(ax, [0, 1])
...
if safeFig
	save("normMSE_OVCorrection.eps", fig)
	save("normMSE_durationEffect.eps", fig2)
	save("normMSE_Noise.eps", fig3)
end
...
#save("simResult_scaledHanning_fivePanel.eps", fig_3)

# ╔═╡ 1ea23222-d66c-4f45-afd4-09fa8c0308a5
begin
	df2 = (x=[0, 4.5], y=[1, 1])
	df3 = (x=[0, 4.5], y=[0, 0])
end

# ╔═╡ 94a4d226-164b-44b6-a632-9996f3d3bb7c
begin
	draw(plt_wOV + data(df2) * visual(Lines) * mapping(:x, :y) + data(df3) * visual(Lines) * mapping(:x, :y))
	ylims!(-0.2, 2.3)
	#save("simResult_scaledHanningWithoutOC.eps", current_figure())
	current_figure()
end

# ╔═╡ f4f32b7f-077b-4763-a599-a7e520455cfe
begin
	draw(plt_wOC + data(df2) * visual(Lines) * mapping(:x, :y) + data(df3) * visual(Lines) * mapping(:x, :y))
	ylims!(-0.2, 2.3)
	#save("simResult_scaledHanningWOC.eps", current_figure())
	current_figure()
end

# ╔═╡ 384e6c4c-b047-4748-8354-3804ce839e0b
begin
	draw(plt_durEF+ data(df2) * visual(Lines) * mapping(:x, :y) + data(df3) * visual(Lines) * mapping(:x, :y))
	ylims!(-0.2, 2.3)
	#save("simResult_scaledHanningDurationEffect.eps", current_figure())
	current_figure()
end

# ╔═╡ 17a8efc6-15c8-4b02-9ca6-effafbfea75b
begin
	draw(plt_Noise + data(df2) * visual(Lines) * mapping(:x, :y) + data(df3) * visual(Lines) * mapping(:x, :y))
	ylims!(-0.2, 2.3)
	#save("simResult_scaledHanningWithNoise.eps", current_figure())
	current_figure()
end

# ╔═╡ b0bd6ccc-9d22-4e6f-8aa1-64bf31568be1
begin
...
begin
fig_3 = Figure()
ax_31 = Axis(fig_3[1,1], title = "No SimOV", xgridvisible = false,
        ygridvisible = false, xticklabelsvisible = false, xticksvisible = false)
ax_32 = Axis(fig_3[2,1], title = "No SimOV wDur", xgridvisible = false,
        ygridvisible = false, xticklabelsvisible = false, xticksvisible = false)
ax_33 = Axis(fig_3[1,2], title = "No OC", xgridvisible = false,
        ygridvisible = false, xticklabelsvisible = false, xticksvisible = false, 			yticklabelsvisible = false, yticksvisible = false)
ax_34 = Axis(fig_3[2,2], title = "OC", xgridvisible = false,
        ygridvisible = false, xticklabelsvisible = false, xticksvisible = false, 			yticklabelsvisible = false, yticksvisible = false)
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "4021801a08acc20b550a7f12c5c6f08c8393dd03"
