...
# ╔═╡ 4eb0ed07-c041-4fae-b10c-7236870290af
begin
using PythonCall
	using CondaPkg
end

# ╔═╡ 41119b1a-e5c8-40cd-89c0-eeb79cc5ab9f
using DSP

# ╔═╡ 1896a6d0-6259-4a51-b70f-03cd9bfc245f
using StatsBase
...
# ╔═╡ ebcab624-9b3f-4499-90ba-f0dd7653dec7
coef_all = cat(coef.(m_all)...,dims=3);

# ╔═╡ 43c679a1-af3d-4193-8865-fe65f8c5f7f4


# ╔═╡ 58e49947-be21-48dd-b3ec-09b613cbad4f
...
# ╔═╡ 0dd77299-73d7-48dc-942f-cae9a51c8ac2
contour()

# ╔═╡ ccd29bf5-bd65-4ae9-a3a2-b82e76c87096
WGLMakie.Page()

# ╔═╡ 2adda886-4f5d-4b0e-a56f-09a700b46196
tval = mean(coef_all,dims=3)[:,2,1]./(std(coef_all,dims=3)[:,2,1]/sqrt(size(coef_all,3)))

# ╔═╡ dbf7ed28-4c7d-441d-b2e9-d4ddb3d8ce7b
plot(tval)
...
# ╔═╡ 41d3057a-3339-4861-b58a-05e86c7863d9
atlas_labels[findall(tval.>6),:]
...
# ╔═╡ f3e780af-afb9-4d7c-9b1a-acb19d2a4898
# ╠═╡ show_logs = false
...
# ╔═╡ 017fea7c-dcd9-46f4-a7e9-a4f9856f97ad
let
	fi = Figure()
	
	
	for (ix,k) = enumerate(20:5:75)
		ix%5
	ax = heatmap(fi[Int(ceil(ix/5)),( ix-1)%5+1], ni[:,k,:,1],axis=(;aspect=1))
	heatmap!(ax.axis, roi4[:,k,:] .* (roi4[:,k,:] .∈ Ref(findall(tval.>6))), alpha = 0.5)
...
# ╔═╡ df884ac0-b6ba-4ba3-9848-70fba1b3db57
extrema(0.5 .* (roi4[:,:,50] .∈ Ref(findall(tval.>6))))
...
begin
	...
	m_all_nlin = Vector{Any}(undef,run_on_n)
	 Threads.@threads for ix in 1:run_on_n
		_ni = do_read_nifti(ix)
		_evts = do_read_tsv(ix)
		m_all_lin[ix] = do_fit(_ni,_evts,@formula(0~1+response_time_centered))
		m_all_nlin[ix] = do_fit(_ni,_evts,@formula(0~1+spl(response_time_centered,5)))
	end
end
	

# ╔═╡ 0f283f97-a987-491f-ab5c-50295240790c
begin
	coefs_lin = extract_coefs(vcat(m_all_lin...), :response_time_centered,"test")
p_values_lin =
    mapslices(c -> pvalue(OneSampleHotellingT2Test(c', [0])), coefs_lin, dims = (3, 4)) |>
    x -> dropdims(x, dims = (3, 4));

coefs_nlin = extract_coefs(vcat(m_all_nlin...), :response_time_centered,"test")
p_values_nlin =
...
# ╔═╡ 6ecca5f8-1b34-4bee-b8f1-4ca401bd00b7
findall(p_values_nlin.<0.05/100)

# ╔═╡ 025f4c45-e2f0-4ed2-bd1a-b66fe4c5cdbf
findall(p_values_nlin.>0.05)

# ╔═╡ 3273f171-ccf4-4f10-8382-eade613ffd95
...
begin
		x_rt = -1:0.1:2
	roi_ix = 1
	tmp = eval_spl.(Ref(x_rt),m_all_nlin,map(x->coef(x)[roi_ix,2:end],m_all_nlin));
	tmp = map((x,m)->coef(m)[roi_ix,1].+x,tmp,m_all_nlin)
	tmp_centered = map(x->x.-mean(skipmissing(x)),tmp)
	
	eval_spline = skipmissing.(eachrow(hcat(tmp...)))
...
f = Figure()
	ax = Axis(f[1,1])
		density!.(ax,map(x->x[1].response_time_centered,Unfold.events.(m_all_nlin)),strokecolor=:black,strokewidth=2,color=(:black,0.))#,bins=-2:0.1:3)
	f
end

# ╔═╡ 0ad4a95a-aaba-4d87-87b7-f24a341259b0
Unfold.events.(m_all_nlin)[1]
...
manifest_format = "2.0"
project_hash = "18b49d591a547acf21e44c37b61e42dcd353c3ad"
...
git-tree-sha1 = "6aedabfa737e647664ae87f933d3678dad5f61d7"
...
git-tree-sha1 = "a446146845b9890b56dd41b7eb3b3e216fd8810f"
...
[[deps.UnfoldStats]]
deps = ["BSplineKit", "Logging", "Random", "StatsAPI", "StatsModels", "Unfold"]
git-tree-sha1 = "a509849ed807531a20aa54383baf886dd126e16e"
uuid = "96fd419a-8306-4ce8-ba5b-cd907cb7647c"
version = "0.2.1"

    [deps.UnfoldStats.extensions]
