function EEG=generate_signal_paper(X,eventTimes,sig,signals,cfg,splinefunction)
...
if nargin == 5
    splinefunction = @(x)log(x);
end
% Generate the signal

separateSignal = zeros(length(signals),length(sig.shape{1}));
sigCount = 0;

data =  zeros(cfg.pnts,1);
for runner = 1:length(eventTimes)
    signalidx = eventTimes(runner):eventTimes(runner)+length(sig.shape{1})-1;
    cutsignalIX = signalidx<cfg.pnts;
    loopIdx = signalidx(cutsignalIX);

    currSig = zeros(length(signals),length(sig.shape{1}(cutsignalIX)));

    for s = 1:length(signals)
        loopSig = sig.shape{s}(cutsignalIX);
        switch signals(s).type
            ...
            case {'1x2','continuous'}
                currSig(s,:) = loopSig .* signals(s).effectsize .* X(runner,s); 
            case 'spline'
                currSig(s,:) = loopSig .* signals(s).effectsize .* splinefunction(X(runner,s)); 
        end
