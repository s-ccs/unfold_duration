...
# ╔═╡ cd761fb0-9ab5-11ef-1e53-3f1f83bd0622
begin
	using CSV
	using UnfoldBIDS
	using DataFrames
...
# ╔═╡ 81f4b863-6097-45f4-ab3b-0f70f6cb7946
using Statistics

# ╔═╡ 3b58c53f-0800-46de-b63b-da7b39d40162
function loadSub(sub,task)
	...
	# Calculate ITI
	events.ITI .= Float64(0)
	for i in 1:size(events, 1)
		# skip first events
		if i == 1; events[i,:ITI] = 0; continue; end;
		
		if events[i, :event_type] == "stimulus"
			tmp = (events[i,:latency] - events[i-1, :latency]) ./ pyconvert(Float64,sfreq) 
			if i+1 > size(events,1); continue; end
			events[i:i+1, :ITI] .= tmp
		end
	end
	
	# Re-reference
	raw = raw.set_eeg_reference(ref_channels=pylist(["P7","P8"]))
...
# ╔═╡ 5b2b010e-b422-47b2-b1d9-28b9b3a58362
begin
	...
	#subList = ["005" "006" "007" "008" "009" "010" "011" "012" "013" "014" "015"]
	#subList = ["005" "006"]
	task = "Oddball"
	chIx = ["Pz"]
end;

# ╔═╡ c25e36f9-ad8e-4d25-8a65-29d23505a592
size(subList)

# ╔═╡ 171b6eea-d6e4-4d83-9a26-a09f996d1c12
begin
...
# ╔═╡ bddcdcbb-d746-466f-87de-a9c6ffbefe11
size(eventsList[1])

# ╔═╡ 2b51ec77-fc09-4a6b-86fb-3cb7a3101097
CodingScheme = "Dummy"

# ╔═╡ 90d8d751-7167-423c-aa06-bfa712ab9545
pyconvert(Float64,sfreq)

# ╔═╡ bd65c184-b04f-48b2-b9f2-d53ea7ffb1b6
if CodingScheme == "Dummy"
	design_lm = [
	"stimulus"=>(
		@formula(0~0+target+distractor+spl(ITI,4)),
		firbasis(τ=(-1,1),sfreq=pyconvert(Float64,sfreq),name="stimulus")),
	"response"=>(
		@formula(0~0+bp_target+bp_distractor),
		firbasis(τ=(-1,1),sfreq=pyconvert(Float64,sfreq),name="response")),
	];
...
# ╔═╡ 564f5e39-1f87-47dc-a2ca-d156a1ed6ca6
size(@rsubset(eventsList[1], :ITI >= 1.2, :ITI <= 1.7))

# ╔═╡ a12039b2-c6d0-4f30-ac65-59f30bc04456
begin
	lm = []
	resultsAll = DataFrame()
	lm_iti = []
	resultsAll_iti = DataFrame()

	for (ix, subject) in enumerate(subList)
		data = pyconvert(Array,EEG_raw[ix].get_data().*1e6)

		lmSub = fit(UnfoldModel,design_lm,eventsList[ix],data,eventcolumn="event_type")
		
		lmSub_iti = fit(UnfoldModel,design_lm,@rsubset(eventsList[ix], :ITI >= 1.2, :ITI <= 1.7),data,eventcolumn="event_type")
		...
		resOne_iti = coeftable(lmSub_iti)
		resOne_iti.subject .= subject
		append!(resultsAll_iti,resOne_iti)
		append!(lm_iti,[lmSub_iti])
	end
end

# ╔═╡ 042288f2-856a-4629-987a-6cac04ba96b9
resultsAll

# ╔═╡ b136f37a-7a68-4ebb-92ba-c5da62f343ca
GA = @chain resultsAll begin
	@subset(:channel .== 26)
	@by([:eventname,:coefname,:time],:estimate=mean(:estimate))
end;

# ╔═╡ 1a190a56-3895-4345-8056-e12ac0e5dcfc
GA_iti = @chain resultsAll_iti begin
	@subset(:channel .== 26)
	@by([:eventname,:coefname,:time],:estimate=mean(:estimate))
end;

# ╔═╡ a65bfd73-8d5b-43d8-a667-a6df12d63731
begin
	...
	GA_bsl = DataFrame()
	# Group by events
	lm_GA_basis = groupby(GA,:eventname)
...
# ╔═╡ f65c3901-8f93-4339-8854-f30293e01bd3
begin
	
	GA_bsl_iti = DataFrame()
	# Group by events
	lm_GA_basis_iti = groupby(GA_iti,:eventname)

	for i in 1:size(lm_GA_basis_iti,1)
		# Group by coefname
		lm_GA_coef_iti = groupby(lm_GA_basis_iti[i],:coefname)
		for j in 1:size(lm_GA_coef_iti,1)
			# Calculate mean of estimates within baseline interval
			mean_bsl_iti = mean(lm_GA_coef_iti[j].estimate[findfirst(lm_GA_coef_iti[j].time .>= bslInterval[1]):findlast(lm_GA_coef_iti[i].time .<= bslInterval[2])])
			# Subtract mean_bsl from all estimates
			bslCorrected_iti = combine(lm_GA_coef_iti[j], :estimate => estimate -> estimate .- mean_bsl_iti)
			append!(GA_bsl_iti,bslCorrected_iti)
		end
	end
	GA_iti.estimate = GA_bsl_iti.estimate_function
end;

# ╔═╡ 462a1bc2-0bda-45f9-896d-d84559998ac5
begin
	GAsubsets = groupby(GA,[:eventname,:coefname])
	keys(GAsubsets)
end;

# ╔═╡ 02d871ee-03ae-40cd-a690-2a71780e462a
begin
	GAsubsets_iti = groupby(GA_iti,[:eventname,:coefname])
	keys(GAsubsets_iti)
end;

# ╔═╡ 758106a8-eb3e-4d94-b38d-23776d3da752
md"""
# Visualization
"""

# ╔═╡ 3a9862ba-0d76-41f9-80e8-ee5bda52b442
md"""
Basis: $(@bind basis_select PlutoUI.Select(unique(GA.eventname)))
"""

# ╔═╡ 7cb0d5d0-4b54-48cd-a44c-183bec41e4f7
begin
	p = AlgebraOfGraphics.data(@subset(GA,:eventname.==basis_select))*mapping(:time,:estimate,color=:coefname) *visual(Lines)
	 x = draw(p, 
		 legend=(position=:right,), 
		 axis=(;limits=(nothing, (-3, 6)))
	 )
end

# ╔═╡ 9d17eb68-7693-4b91-98aa-ff45a7ca877a
AlgebraOfGraphics.data(@subset(GA,:eventname.==basis_select))*mapping(:time,:estimate,color=:coefname) *visual(Lines)|>plt->draw(plt,legend=(position=:right,), axis=(;limits=(nothing, (-3.5, 6.5))))

# ╔═╡ 448d6c1a-1be4-42b7-8ea8-acd37572f81e
AlgebraOfGraphics.data(@subset(GA_iti,:eventname.==basis_select))*mapping(:time,:estimate,color=:coefname) *visual(Lines)|>plt->draw(plt,legend=(position=:right,), axis=(;limits=(nothing, (-3.5, 6.5))))

# ╔═╡ fbc51a88-84e8-47b0-8363-16b7656f1c81
lm[1]

# ╔═╡ 3d30cdec-cb9c-4560-b9d8-bf2557b2b816
eff = effects(Dict(:coefname => ["target", "distractor"],:ITI => 0.8:0.2:1.6), lm[6])

# ╔═╡ 253ce39f-fca8-493e-a181-fd71fe195b48
begin
	plot_erp(
	    @rsubset(eff, :channel == 26, :eventname == "stimulus");
	    mapping = (; color = :ITI  => nonnumeric)
	)
end

# ╔═╡ e3eae56b-9f50-43d3-8454-7e81b1488a59
unique(eff.eventname)

# ╔═╡ 3229bb71-54b2-435f-a2b5-e834ea695cf5
let
if CodingScheme == "Dummy"
...
manifest_format = "2.0"
project_hash = "5a41e7cdd9d649ebc00e71a0241f6a836e3ae631"
...
git-tree-sha1 = "14faed094d3728f4e549b6fbc4b38b9b8c6a4a99"
...
git-tree-sha1 = "b3a1aa4dbc847744e479012fd78d5c8fad13b5f2"
...
git-tree-sha1 = "a24dcd72b75b13a7d8a90c21a4a1d1a23e6db576"
