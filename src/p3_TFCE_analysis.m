...
% Format will be chan X data x subject
...
for modelRT = [3 2]
    ...
    dataTarget = zeros(1, size(fn_p3{1,'beta'},3), length(tmp_ix)); 
    dataDistractor = zeros(1, size(fn_p3{1,'beta'},3), length(tmp_ix)); 
    dataTarget_beta = zeros(1, size(fn_p3{1,'beta'},3), length(tmp_ix)); 
    dataDistractor_beta = zeros(1, size(fn_p3{1,'beta'},3), length(tmp_ix)); 
    ...
    if modelRT == 3
        dataNoRT{1} = dataTarget;
        dataNoRT{2} = dataDistractor;
        dataNoRT{3} = dataTarget_beta;
        dataNoRT{4} = dataDistractor_beta;
    else
        dataRT{1} = dataTarget;
...
%%
Results = ept_TFCE(DataFile1, DataFile2, ElecFile, 'rsample', 512, 'chn', chanNeighbours);