...
Here, we first show that failing to explicitly account for event durations can lead to spurious results and thus are important to consider. Next, we propose and compare several methods based on multiple regression to explicitly account for stimulus durations. Using simulations, we find that non-linear spline regression of the duration effect outperforms other candidate approaches. Finally, we show that non-linear event duration modelling is compatible with linear overlap correction in time, making it a flexible and appropriate tool to model overlapping brain signals. 
...
We analyzed the generated continuous time series using rERP linear deconvolution based on multiple regression. We varied three analysis parameters: overlap correction (yes/no), modelling duration (simple averaging vs. to model duration), and type of duration-modelling (four different approaches to model duration, see below), effectively forming a 5x2 design of analyses. However, the classical averaging was only calculated in order to compare duration modelling to the current standard practice of not modelling duration at all, as such we show all results relative to the classical averaged ERP (Wilkinson Formula: $y ~ 1$), which ultimately left us with a 4x2 design (overlap correction x type-of-model).
...
Third and fourth, we make use of the generalized additive modelling framework (GAM) and use a regression B-spline with two different levels of model flexibility (Wilkinson Formula: $y ~ 1 + italic("spline"("dur", "df"=5))$, and more flexibly: $y ~ 1 + italic("spline"("dur", "df"=10))$, termed duration-as-5spline and duration-as-10spline). 
...
Additionally to the five modelling approaches, we tested the influence of actively modeling overlap using linear deconvolution using the FIR-deconvolution approach popularized through fMRI @ehinger.dimigen_2019 @smith.kutas_2015a, against the passive method to try to resolve overlap via the duration coefficients @nikolaev.etal_2023.
...
2) Comparing the top and bottom rows, without and with overlap correction, shows that only with overlap correction we can get close to recovering the original shape. Thus, purely modelling duration effects, cannot replace overlap correction.
...
By applying this analysis to real data, we underscore the importance of modelling event durations both, in cases where they are of interest themselves, as well as in situations where durations may have an interfering influence on the results. 
...
- René Skukies: manuscript; figures; simulations; data analysis;

- Judith Schepers: FRP analysis; review

- Benedikt Ehinger: project idea; supervision; simulations; FRP analysis; review
