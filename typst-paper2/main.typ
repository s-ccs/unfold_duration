...
Having a method to tackle the duration problem still leaves us with the issue of overlap between adjacent events, however. For instance, the stimulus ERP in an active oddball task, which is typically longer than 600ms, will necesarily overlap in time with the response ERP, which on average occurs after 430ms for distractor stimuli (see @Problem\.A-C). Prior research has shown such overlap, can be addressed within the same regression framework that is used to solve the covariate problem, by using linear deconvolution modelling @ehinger.dimigen_2019 @smith.kutas_2015a. This approach has been successfully applied to several experiments, with application to free viewing tasks (e.g. @coco.etal_2020 @gert.etal_2022 @nikolaev.etal_2023 @welke.vessel_2022), language modelling @momenian.etal_2024, auditory modelling @skerritt-davis.elhilali_2018 and many other fields.
...
To show the validity of our approach, we make use of systematic simulations and compare spline-regression with multiple alternative ways of modelling event duration. We further explore  potential interactions with overlapping signals. Lastly, we demonstrate the implications of our approach by applying it to a real free-viewing dataset.


#figure(
  image("assets/20241002Figure2_full.svg", width: 100%),
...
For the second approach, we binned the event durations into their 10 quantiles, and estimated a separate ERP for each of them, (Wilkinson Formula: $y ~ 1 + italic("bin"("dur"))$, termed duration-as-categorical). This was done as it, at first sight, is a simple solution to modelling varying event durations, even though it is heavily criticized by in the statistics community as “dichotomania” @greenland_2017, and especially in pharmaceutics and the medical field @giannoni.etal_2014 @senn_2005. In our case, we introduce arbitrary bins, and thus unrealistic discontinuities at the event-duration bin edges. Yet, this practice has previously been presented as a favourable analysis over classical ERP averaging, and leading to an increase in power @poli.etal_2010.
...
To visualize our results in a topoplot series @mikheev.etal_2024a, we break down the multi-parameter spline estimate to a single value per channel and time-point, we calculate the maximal marginal effect over the range durations calculated before. This quantity is positively biased (the maximum needs to be >0). We therefore decided to test the coefficients directly (as described above), rather than this visualization. This has the unfortunate effect, that in some cases, what is tested, is not the same as what is visualized, the reason being the lacking visualization tools for non-linear effects. Only for the difference do visualization and test directly correspond to each other. We used the UnfoldMakie.jl @mikheev.etal_2024b and Makie.jl @DanischKrumbiegel2021 libraries for visualization.
...
#cite(<nikolaev.etal_2016>, form: "prose") and #cite(<vanhumbeeck.etal_2018>, form: "prose") raised the idea that overlap effects can be taken into account, using only a non-linear spline for the duration predictor, but never explicitly tested this in simulations. As visible in @MainResults\.D (and compare not overlap corrected with overlap corrected results in @Quali above), this is not sufficient. In fact, not-modelling the overlap via duration even outperforms modelling it in our simulation setting. This shows us that we indeed have to, and can, rely on other techniques like linear deconvolution, as proposed in this study.

=== Influence of Shape and Overlap Distribution

There are two more parameters of our simulations that we have not yet presented: overall shape of the ERP (not only the duration part), and distribution of durations (and with that, overlap). The overall main conclusions hold true regardless of the set parameters.
...
We move our eyes approximately 120-240 times each minute. Besides being an interesting behaviour in itself, eye movements offer a unique and data-rich segmentation event for EEG. Consequently, more and more researchers use EEG and eyetracking to better understand human cognition @dimigen.etal_2014 @dimigen.ehinger_2021 @nikolaev.etal_2016. Here, EEG and eye movements are co-registered, such that single fixation can be used as event markers in the EEG. These fixations can then be assigned to different conditions based on the fixated stimulus, and modelled accordingly to achieve a fixation related potential.
...
- #underline[Block-design]: As described in @block, a continuous stream of overlapping events without breaks leads to increased overfit under certain simulation conditions. Introducing “inter-block-intervals”, where at least two adjacent events break the homogeneous overlap-duration relationship, alleviates this issue. We think that such pauses break the correlation between inter-event-onsets as modelled with the FIR time expansion, and the duration predictor. Further simulations showed that this effect is not specific to splines, but also occurs with linear duration modelling. Further, this effect is irrespective of shape and occurs in pure noise data as well (see @BlockResults). However, by adding only one or more (depending on SNR) of the aforementioned inter-block-intervals, we could resolve this noise amplification.
 ...
 In real data such inter-block-intervals can occur, for example in free viewing experiments where fERPs are of interest. Here, while fixation duration and overlap are indeed perfectly correlated within the e.g. 5s of looking at a stimulus, the last fixation before removing the stimulus does have a duration, but no further (modelled) fixation overlapping. In other experiments, blocks can (and usually are) in the experiment by design.
...
Additionally, our simulation study covers a large area of simulation parameters, but cannot cover all possible cases. Indeed, some interesting variations could be to introduce continuous and categorical condition differences, multiple different event types, use more flexible non-linear analysis methods (e.g. DeepRecurrentAutoencoder @chehab.etal_2022) or try to simulate empirically found duration effects more realistically.
...
Another clear limitation of our study is, that we only look at model performance of what essentially constitutes results from a single subject. We do not address in this paper how the performance will improve if estimates over multiple subjects are aggregated. In the applied example we average results over multiple subjects, but here we want to highlight some a potential caveat relating to spline modelling in general: Experimenters need to decide whether to normalize durations per subject, effectively modelling quantiles over subjects, or assume that ERPs relate better to absolute durations. Because knot-placement and coverage of splines depend on the distribution of each one single subject, not all subjects have the same  smoothing factor. Interested readers are pointed to @wood_2017 for a discussion of these issues. 
...
In the present study, we show through extensive simulations that event durations can and should be modelled in conjunction with overlap correction. Through combining linear overlap correction with non-linear event duration modelling, researchers gain a more powerful toolset to better understand human cognition. Overlap correction disentangles the true underlying signals of the ERPs, while event duration modelling can lead to a more nuanced understanding of the neural responses. 

By applying this analysis to real data, we underscore the importance of modelling event durations both, in cases where they are of interest itself, as well as in situations where durations may have an interfering influence on the results. 
...
All code is publicly available at https://github.com/s-ccs/unfold_duration. The fERP data can be obtained from Gert et al. (2022). 
...
1. Data was downsampled to 500Hz
...
3. Data was re-referenced to avg reference
...
6. Bad components were marked using ICLabel with Parameters only set for eye movements and muscle artefacts
...
Additionally, during simulations the data was downsampled (default 100Hz) and high-pass filtered (default 0.5 Hz) according to simulation instance.

#pagebreak()
== No Noise Results <NoNoiseAp>

#figure(
  image("assets/20240923ResultsFigureNoNoise_B.svg", width: 100%),
