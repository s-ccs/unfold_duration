...
Here, we first show that failing to explicitly account for event durations can lead to spurious results and thus are important to consider. Next, we propose and compare several methods based on multiple regression to explicitly account for stimulus durations. Using simulations, we find that non-linear spline regression of the duration effect outperforms other candidate approaches. Finally, we show that non-linear event duration modeling is compatible with linear overlap correction in time, making #text()[mass-univariate models combining non-linear spline regression of duration and linear overlap correction] a flexible and appropriate tool to model overlapping brain signals. 
This allows us to reconcile the analysis of brain responses to stimuli in #text()[situations where durations differ between conditions, e.g. different reaction times, different stimulus durations, or fixation-related potentials with different fixation durations.]
While in this paper we focus on EEG analyses, #text()[we additionally show that our] findings generalize #text()[to fMRI BOLD-responses, and argue that they should generalize to other overlapping signals such as LFPs, or pupil dilation responses.]
  ],
  keywords: ("event duration", "EEG", "fMRI", "regression ERP", "overlap correction", "GAM"),
...
To show the validity of our approach, we use systematic simulations and compare spline regression with multiple alternative ways of modeling event duration. We further explore potential interactions with overlapping signals. Lastly, we demonstrate the implications of our approach by applying it to a real free-viewing dataset #rev[as well as to an fMRI stroop task].


#figure(
  image("assets/20241119Figure2.svg", width: 100%),
  caption: [Potential influence of reaction time in an oddball task. (A) Experimental sequence of the classical active oddball task; #rev[Because the next stimulus is shown only after the response of the participants, their reaction time controls the duration of a trial]. (B) Observed EEG during the task (top) and underlying single-event responses #rev[(bottom; solid lines = stimulus onset/ERP, dashed line = response onset/ERP)]. (C) Distribution of reaction times between conditions of a single subject in an active oddball task from the ERP core dataset @kappenman.etal_2021. (D) Simulated isolated response to stimulus and response, including the effect of reaction time on the stimulus response. (E) Results from a classical average ERP analysis. Many artefactual differences are visible. (F) Average ERP after overlap correction. (G) ERP after overlap correction and covariate control through general additive modeling, showing that a condition effect is only attributed to a difference in reaction time distribution (E-G are results from simulated data). For a similar figure showcasing fixation-related potentials and the effect of fixation duration, see Figure 2 in #cite(<dimigen.ehinger_2021>, form: "prose") and for face-related activity see #cite(<ehinger.dimigen_2019>, form: "prose").],
) <Problem>
...
#figure(
  muchpdf(
    read("assets/20250702SimMethods.pdf", encoding: none),
    width: 100%,
  ),
  caption: [Simulation methods; (A) Three different “proto-ERPs” (i.e. ground truths). #rev[In a single simulation instance only one of the three "proto-ERPs" was used as the ground truth. The color indicates the effect of duration on the ERP.]; (B) A single continuous simulation with “scaled hanning” #rev[(A, left panel)] as ground truth with the underlying single overlapping ERPs. #rev[Vertical bars underneath the signal indicate event onsets.] Notice the perfect correlation between event distance and duration, i.e. the longer the distance between two events, the more scaled is the hanning shape; (C) the same continuous simulation with noise added.]
)<Methods>
...
Third and fourth, we use the generalized additive modeling framework (GAM) and use a regression B-spline (#rev[please see #cite(<ehinger.dimigen_2019>, form: "prose") for a more practical, and #cite(<smith.kutas_2015>, form: "prose") for a more theoretical explanation on the use of GAMs and B-splines within EEG]) with two different levels of model flexibility (Wilkinson Formula: $y ~ 1 + italic("spline"("dur", "df"=5))$, and more flexibly: $y ~ 1 + italic("spline"("dur", "df"=10))$, termed duration-as-5spline and duration-as-10spline). #rev[There doesn't exist a physiological explanation to choose exactly these number of splines. The number of splines solely depends on a trade-off between model flexibility and over fit, and currently there is no method implemented to estimate this automatically. Here, we chose a lower (5) and higher (10) number of splines to showcase the trade off between model flexibility and overfit, as we expected the 10 spline model to show more variance in MSE results and/or worse MSE values due to overfit.]
...
We simulated ERPs with varying event durations and tried to recover the ground truth using four different modeling strategies: duration-as-linear, duration-as-categorical, duration-as-5splines, and duration-as-10splines. 
...
1) The overall best recovery of the ground truth is achieved in the lower row, first panel from the right, the duration-as-10 splines predictor.
...
To showcase our approach, we then calculated fixation-related potentials (FRP) using custom scripts and the Unfold.jl toolbox @ehinger.alday_2024. The data was modeled using #rev[a linear model with fixations following the Wilkinson notation]: 
...
Where _y_fixation_ is the recorded activity related to a fixation, _is_face_ represents whether in that fixation a face was fixated (yes/no), _duration_ the fixation duration; _sac_amplitude_ the saccade amplitude; _fix_avgpos_x_ & _fix_avgpos_y_  the average x- and y-position of the current fixation. #rev[Apart from] fixation duration, we incorporated the other covariates in the analysis as they are known to have an influence on the FRP @gert.etal_2022 @nikolaev.etal_2023, and we wanted to adjust for these factors when investigating fixation duration. Additionally, #rev[the linear model lets us incorporate a different basis function to model stimulus onsets (i.e. the appearance of a picture on screen; for an explanation on how different basis functions for different events are combined in one linear model please see #cite(<ehinger.dimigen_2019>, form: "author"), #cite(<ehinger.dimigen_2019>, form: "year")) using a] 
$ italic("y_stimulus") ~ 1 $
formula, as the onset of a stimulus can have huge overlapping effects on subsequent fixations @coco.etal_2020 @dimigen.ehinger_2021 @gert.etal_2022. #rev[Incorporating the stimulus onset in our model in this way lets us account for the overlap the stimulus response has with any subsequent fixation.]
...
However, when modeling overlap in addition to duration, we found duration effects to become smaller and even vanish. This indicates, as proposed by #cite(<nikolaev.etal_2016>, form: "prose"), that a duration predictor tries to compensate for the overlap (but ultimately fails to do so; see simulation results @results). Yet, an effect over central electrodes at around 0.450 s. after fixation onset still persists in the results after overlap correction (@AppliedResuts). This is direct evidence for the existence of event duration effects in EEG-eye-tracking co-registered data. #rev[Finally, it should be noted here that these results seem much less noisy than our simulation results @Quali. The reason being that here we aggregated over a group of subjects, whereas our (single) simulation results essentially compare to a single subject analysis.]

#figure(
  image("assets/figure2_unfoldduration.svg", width: 100%),
  caption: [Results from a free viewing experiment. (top) Regression FRPs with marginal effects of fixation duration from channels AFz [left], Cz [middle], and Oz [right], once without [up] and once with [bottom] overlap correction. (bottom) Topoplots for results without overlap correction [up], with overlap correction [middle], and the difference between with and without overlap correction [bottom]; Displayed values are the maximal difference between two evaluated marginal effects (i.e. the maximal difference between any line in top), #rev[As a statistical test, we used a Hotellings-t#super[2] test on all coefficients of the non-linear spline coefficients, not only the maximal difference, which was used only for the display.] The significance of the difference was tested with mass-univariate one-sample t-tests, and corrected for multiple comparisons using FDR. We plot a marker, only if the minimal corrected p-value in a time window is below 0.05 and weight its size logarithmically according to the p-value (that is, larger markers indicate smaller p-values).],
)<AppliedResuts>
...
To showcase that our approach generalizes to modalities other than EEG, we reanalyzed a subset of the fMRI data used by #cite(<mumford.etal_2023a>, form: "prose"), their Stroop task.
...
Currently, penalized splines are not defined; thus, we manually fix the number of splines to five. A higher number of splines (and thus a higher flexibility in the possible non-linearity; we tried 10 splines) resulted in strong overfit and thus much reduced significances. This was unsurprising to us, given that we only have 10 minutes of fMRI data per subject and no pooling over subject was used in the estimation process. 

As in the free viewing analysis, we use Hotelling-T² test to test the spline coefficients against the statistical null hypothesis of them being 0. We finally calculated a Benjamini-Yekutieli false discovery rate correction, via MultipleTesting.jl @benjamini.yekutieli_2001 @gehring.etal_2023), over the 100 ROIs and 24 timepoints.
...
Our simulations indicate that non-linear event duration modeling and linear #rev[overlap correction] of adjacent events can be seamlessly integrated. We also showed that this comes at a minimal cost in terms of potential overfit, should there be no ground truth duration effect in the data. We therefore recommend using our proposed non-linear duration modeling in cases researchers suspect effects of duration.
...
In our paper, we apply our approach to two datasets from two different modalities: EEG and fMRI. In both datasets we demonstrate a significant effect of duration (fixation duration for EEG, reaction time for fMRI).

=== EEG

#rev[In direct comparison to our simulation results, we first applied our approach on FRPs in an unrestricted viewing dataset. Given that fixations differ in durations, with an average of ~230ms, but up to a second, we expected that the evoked potential is modulated by duration as well. And indeed, without overlap correction, we clearly found strong non-linear duration effects. However, most of the effect was better explained by overlap rather than duration effects. Subsequent analysis still showed a significant duration effect, but it was comparatively small in absolute effect size. This was surprising as one earlier study @nikolaev.etal_2023@ehinger.etal_2018 found a larger effect size of duration for a similar analysis. To better characterize and understand this effect, more datasets should be analyzed; nevertheless, we can conclude that we successfully disentangled overlap from fixation duration effects.]

=== fMRI

#rev[In @fMRI we directly show that our approach is applicable to] functional magnetic resonance imaging (fMRI). fMRI has the advantage that the underlying measure generator is relatively well understood (as the hemodynamic response function; HRF). As such, the problem of overlapping signals has been thoroughly addressed @penny.etal_2011. Yet, while some studies have shown the importance of linearly modeling reaction times in fMRI studies @mumford.etal_2023a @yarkoni.etal_2009, this is not common practice yet. #rev[Adding to this, we show in @fMRI that one can (and should) model reaction time via non-linear spline regression instead of linear estimation.] Further, both studies only focus on reaction time, whereas we argue to generalize such modeling to other event durations, such as stimulus durations or fixation durations.

One possible different approach to deal with the problem of varying durations was proposed by #cite(<jackgrinband.etal_2008>, form: "prose"). In their study, the authors come up with two different models incorporating duration to estimate the BOLD signal: One model uses "variable duration epochs" (duration modulated boxcars) to convolve the HRF; the other uses the more common constant impulse response function (IRF) with an additional regressor linearly modulated by duration (@fMRI_results\.C, black lines). The authors found that both their approaches increased statistical power, consistency, and interpretability of results. However, these models are only able to either capture the time-aspect (variable epoch) or the amplitude-aspect (constant IRF with regressor) of the duration effect. Our approach improves upon their models in two ways: First, our approach is able to capture duration effects in both time and amplitude. And second, our model is able to incorporate non-linear effects, a refinement that was already proposed by #cite(<jackgrinband.etal_2008>, form: "prose").
]
...
All code is publicly available at https://github.com/s-ccs/unfold_duration. The FRP data can be obtained from #cite(<gert.etal_2022>, form: "prose"). #rev[Raw fMRI data can be obtained from https://openneuro.org/datasets/ds004636/, for the preprocessed data please refer to the corresponding author of #cite(<mumford.etal_2023a>, form: "prose").]

= Author Contributions

- #underline[René Skukies]: Conceptualization; Methodology; Software; Formal analysis (simulations, fMRI analysis); Data Curation; Writing - Original Draft; Visualization

- #underline[Judith Schepers]: Formal analysis (FRP analysis); Writing - Review & Editing

- #underline[Benedikt Ehinger]: Conceptualization; Methodology; Software; Resources; Supervision; Formal analysis (simulations, FRP analysis, fMRI analysis); Writing - Review & Editing; Visualization; Funding acquisition
...
#rev[We want to specifically thank Jeanette Mumford for sharing her pre-processed fMRI dataset with us.]
